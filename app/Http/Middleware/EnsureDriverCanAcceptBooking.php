...
class EnsureDriverCanAcceptBooking
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($user === null || ! $user->isDriver()) {
            return ApiResponse::error('FORBIDDEN', 'Only drivers can accept bookings.', 403);
        }

        if (! $user->tokenCan('booking:accept:self')) {
            return ApiResponse::error('FORBIDDEN', 'Missing booking:accept:self permission.', 403);
        }
