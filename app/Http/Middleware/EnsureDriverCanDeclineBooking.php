...
class EnsureDriverCanDeclineBooking
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($user === null || ! $user->isDriver()) {
            return ApiResponse::error('FORBIDDEN', 'Only drivers can decline bookings.', 403);
        }

        if (! $user->tokenCan('booking:decline:self')) {
            return ApiResponse::error('FORBIDDEN', 'Missing booking:decline:self permission.', 403);
        }
