...
class EnsureDriverCanEndTrip
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($user === null || ! $user->isDriver()) {
            return ApiResponse::error('FORBIDDEN', 'Only drivers can end trips.', 403);
        }

        if (! $user->tokenCan('trip:end:self')) {
            return ApiResponse::error('FORBIDDEN', 'Missing trip:end:self permission.', 403);
        }
