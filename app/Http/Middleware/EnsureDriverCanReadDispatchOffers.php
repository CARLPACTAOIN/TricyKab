...
class EnsureDriverCanReadDispatchOffers
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($user === null || ! $user->isDriver()) {
            return ApiResponse::error('FORBIDDEN', 'Only drivers can access this resource.', 403);
        }

        if (! $user->tokenCan('booking:offer:read:self')) {
            return ApiResponse::error('FORBIDDEN', 'Missing booking:offer:read:self permission.', 403);
        }
