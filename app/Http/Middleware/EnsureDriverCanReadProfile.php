...
class EnsureDriverCanReadProfile
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($user === null || ! $user->isDriver()) {
            return ApiResponse::error('FORBIDDEN', 'Only drivers can read driver profile.', 403);
        }

        if (! $user->tokenCan('compliance:read:self')) {
            return ApiResponse::error('FORBIDDEN', 'Missing compliance:read:self permission.', 403);
        }
