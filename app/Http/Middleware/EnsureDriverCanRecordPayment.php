...
class EnsureDriverCanRecordPayment
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($user === null || ! $user->isDriver()) {
            return ApiResponse::error('FORBIDDEN', 'Only drivers can record payments for trips.', 403);
        }

        if (! $user->tokenCan('payment:record:self')) {
            return ApiResponse::error('FORBIDDEN', 'Missing payment:record:self permission.', 403);
        }
