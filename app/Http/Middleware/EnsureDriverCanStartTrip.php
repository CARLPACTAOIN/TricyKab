...
class EnsureDriverCanStartTrip
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if ($user === null || ! $user->isDriver()) {
            return ApiResponse::error('FORBIDDEN', 'Only drivers can start trips.', 403);
        }

        if (! $user->tokenCan('trip:start:self')) {
            return ApiResponse::error('FORBIDDEN', 'Missing trip:start:self permission.', 403);
        }
