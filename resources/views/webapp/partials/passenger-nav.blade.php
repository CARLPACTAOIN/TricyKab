...
@endphp
<nav class="fixed bottom-0 left-0 right-0 z-30 border-t border-slate-200 bg-white" aria-label="Passenger primary">
    <div class="mx-auto flex h-16 w-full max-w-6xl items-stretch justify-around px-2 sm:px-4">
        <a class="{{ $item('book') }}" href="{{ route('passenger.app') }}">
            <span class="material-icons-outlined text-base">pin_drop</span>
            <span>Book</span>
        </a>
        <a class="{{ $item('trips') }}" href="{{ route('passenger.trips') }}">
            <span class="material-icons-outlined text-base">history</span>
            <span>Trips</span>
        </a>
        <a class="{{ $item('profile') }}" href="{{ route('passenger.profile') }}">
